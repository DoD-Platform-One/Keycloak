...
1. Update /chart/Chart.yaml to the appropriate versions.
    ...
    ```
1. Update the /README.md following the [gluon library script](https://repo1.dso.mil/platform-one/big-bang/apps/library-charts/gluon/-/blob/master/docs/bb-package-readme.md). You should update the README again at the end to pick up any additional chart changes you make during development testing.
...
1. If you are upgrading the Keycloak version follow instructions in the [P1 plugin repo](https://repo1.dso.mil/platform-one/big-bang/apps/product-tools/keycloak-p1-auth-plugin/-/blob/main/docs/DEVELOPMENT_MAINTENANCE.md) to upgrade the custom plugin.
1. Test the plugin changes locally using the [docker-compose local dev environment](../development/README.md).
...
1. Build and publish a plugin image to the [Big Bang Keycloak container registry](https://repo1.dso.mil/platform-one/big-bang/apps/product-tools/keycloak-p1-auth-plugin/container_registry/) using the instructions in the [P1 plugin repo](https://repo1.dso.mil/platform-one/big-bang/apps/product-tools/keycloak-p1-auth-plugin/-/blob/main/docs/deployment.md). The image label should have "test" in the name like this `test-X.X.X`. 
...
1. Make an official semver release tag in the p1 plugin repo and monitor the mirrored [Party Bus IL2 pipeline](https://code.il2.dso.mil/platform-one/big-bang/keycloak/keycloak-p1-auth-plugin/-/pipelines) to make sure it passes.
1. After all testing locally and k8s testing with a BigBang deployment has been completed publish an official plugin image in [IronBank](https://repo1.dso.mil/dsop/big-bang/p1-keycloak-plugin). The order that things should happen with BigBang MRs, BigBang release, and publishing plugin image in IronBank is still unclear. Brief instructions for IronBank pipeline maintenance:
    ...
    1. update hardening_manifest.yaml
        ...
        - resources.url
        - resources.validation.value (use sha256sum to get the hash value)
    ...
    1. Monitor the issue to make sure it keeps moving.
